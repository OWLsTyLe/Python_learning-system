...
export class DashboardPageComponent implements OnInit {
  ...
  // --- Останній тест по кожному топіку ---
  const quizByTopic: Record<number, { score: number; total: number }> = {};
  for (const q of quizzes) {
    // якщо проходив кілька разів — беремо найкращий результат
  ...
  // --- Прогрес по топіках: 50% уроки + 50% тест ---
  ...
  // --- Сертифікатів (100% = всі уроки + тест на 100%) ---
